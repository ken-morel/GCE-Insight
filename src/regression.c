...
RegressionModel* init_regression_model(size_t n_features, float learning_rate) {
    ...
    for (size_t i = 0; i <= n_features; i++) {
        model->theta[i] = ((float)rand() / RAND_MAX) * 0.1f - 0.05f; // Small random values between -0.05 and 0.05
    }
...
// We'll keep the original gradient descent function as is, since train_model now has its own
// implementation with normalization. This keeps the function available for other uses.
void gradient_descent_step(RegressionModel *model, GCEDataCollection *data) {
    if (!model || !data || data->sample_count == 0) return;
...
float compute_cost(RegressionModel *model, GCEDataCollection *data) {
    if (!model || !data || data->sample_count == 0) return 0.0f;
...
void train_model(RegressionModel *model, GCEDataCollection *data) {
    ...
    for (size_t j = 0; j < model->n_features; j++) {
        ...
        model->feature_ranges[j] = (max_val == min_val) ? 1.0f : (max_val - min_val);
    ...
    for (size_t i = 0; i < data->sample_count; i++) {
        normalized_features[i] = malloc((model->n_features + 1) * sizeof(float)); // +1 for bias
        ...
        for (size_t j = 0; j < model->n_features; j++) {
            normalized_features[i][j + 1] = (data->training_samples[i].X[j] - model->feature_means[j]) / model->feature_ranges[j];
        }
    }
...
float predict(RegressionModel *model, float *features) {
    ...
    float prediction = model->theta[0]; // Bias term
    for (size_t i = 0; i < model->n_features; i++) {
        float normalized_feature = (features[i] - model->feature_means[i]) / model->feature_ranges[i];
