...
// Visualization drawing functions - only defined in main.c where gama.h is included
void draw_visualization() {
    // Draw chart area
    gm_draw_rectangle(vis_data.left, vis_data.top,
                 vis_data.right - vis_data.left,
                 vis_data.bottom - vis_data.top,
                 gm_set_alpha(GM_WHITE, 10));
    ...
    if (vis_data.data && vis_data.data->record_count > 0) {
        ...
        for (int year = start_year; year <= end_year; year++) {
            if (year % 2 == 0) { // Label every other year to avoid crowding
                double x_pos = map_value(year, start_year, end_year,
                ...
                gm_draw_line(x_pos, vis_data.bottom, x_pos, vis_data.top,
                            0.002, gm_set_alpha(GM_WHITE, 30));

                // Year label
                gm_draw_text(x_pos, vis_data.bottom - 0.08, year_str, "", 0.03, GM_GRAY);
            }
        }

        // Y-axis labels (Pass %)
        for (float percent = 0; percent <= 100; percent += 20) {
            double y_pos = map_value(percent, 0, 100,
                                    vis_data.bottom, vis_data.top);

            // Horizontal line for each percentage
            if (percent > 0 && percent < 100) {
                gm_draw_line(vis_data.left, y_pos, vis_data.right, y_pos,
                            0.002, gm_set_alpha(GM_WHITE, 30));
            }
    ...
    if (vis_data.show_predictions && vis_data.data && vis_data.data->record_count > 0) {
        ...
        GCE_Subject_Record last_record = vis_data.data->records[vis_data.data->record_count - 1];

        double last_x = map_value(last_record.year, start_year, end_year,
                                 vis_data.left, vis_data.right);

        double last_y = map_value(last_record.pass_percent, 0, 100,
                                 vis_data.bottom, vis_data.top);

        gm_draw_line(last_x, last_y, pred_x, pred_y, 0.007, gm_set_alpha(GM_RED, 150));

        // Draw prediction label
        char pred_str[30];
        sprintf(pred_str, "%d: %.2f%%", vis_data.prediction_year, vis_data.predicted_pass_percent);
...
void draw_data_info() {
    ...
    if (vis_data.predicted_pass_percent > 0) {
        char pred_info[100];
        sprintf(pred_info, "Predicted %d Pass %%: %.2f%%",
                vis_data.prediction_year, vis_data.predicted_pass_percent);
        gm_draw_text(0, 0.85, pred_info, "", 0.04, GM_YELLOW);
    }

    // Draw controls information
    gm_draw_text(0, -0.9, "Controls: [R] Reset, [S] Show/Hide Data, [P] Show/Hide Predictions",
...
int main() {
  ...
  // Load sample data
  load_gce_data(data_collection, "gce_data.csv");  // Note: file parameter is currently ignored in our implementation
  ...
  do {
    ...
    if (gm_key_pressed('c', 'r')) {  // Reset on 'R' key
        // Reload data
        cleanup_gce_data_collection(data_collection);
        data_collection = init_gce_data_collection();
        load_gce_data(data_collection, "gce_data.csv");
    }
    if (gm_key_pressed('c', 's')) {  // Toggle data points on 'S' key
        vis_data.show_data_points = !vis_data.show_data_points;
    }
    if (gm_key_pressed('c', 'p')) {  // Toggle predictions on 'P' key
        vis_data.show_predictions = !vis_data.show_predictions;
    }

    // Clear the screen
    gm_draw_rectangle(-1, 1, 2, 2, GM_BLACK); // Full background
